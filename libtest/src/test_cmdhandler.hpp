...
TEST_CASE("HSB can be parsed from Strings with a given HSB", "[CmdHandler]") {
    ...
    SECTION("when just hsb values are given without key") {
        HSB result = CmdHandler::hsbFromString(givenHsb, "12,23,34");
